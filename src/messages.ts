...
export const providerFailureMessages: Record<string, string> = {
  ...
  "provider.server_error": "供应商服务暂时不可用。",
...
export const wslFailureMessages: Record<string, string> = {
  ...
  "wsl.schema_unknown": "WSL2 配置使用未知的管理 schema，普通应用已停止。",
  "wsl.markers_invalid": "WSL2 配置的管理区块标记损坏、重复或顺序错误。",
  "wsl.managed_metadata_invalid": "WSL2 配置的管理元数据缺失、重复或无效。",
  "wsl.managed_fields_invalid": "WSL2 配置的受管字段缺失或值无效。",
  "wsl.credential_reference_invalid": "WSL2 配置引用了无效或危险的命令式凭据路径。",
  ...
  "wsl.reclaim_confirmation_required": "重新接管前需要再次确认。",
  "wsl.reclaim_start_authorization_required": "核验已停止的 WSL2 发行版前需要明确确认临时启动。",
  "wsl.reclaim_not_required": "该 WSL2 配置当前不需要重新接管，请刷新实际状态。",
  "wsl.rollback_failed": "WSL2 配置写入失败，且无法确认已恢复原配置。请停止继续操作并查看问题日志。",
...
export const providerMessages = {
  ...
  visitDaywayWebsiteAccessibleName: "访问 DayWay 官网获取 API Key",
  daywayApiKeyHint: "还没有 API Key？",
  daywayApiKeyLink: "前往 DayWay 官网获取 API Key",
  ...
  wslNoProvider: "供应商目录中没有已验证供应商。",
  wslNoCurrentProvider: "该 WSL2 发行版尚无当前供应商。",
  wslCatalogCount: (count: number) => `供应商目录：${count} 个已验证供应商。`,
  ...
  wslApply: "应用到 WSL2",
  wslReclaim: "备份并重新接管",
  wslReclaiming: "正在重新接管",
  wslReclaimed: (provider: string, distro: string) =>
    `已备份并由“${provider}”重新接管 WSL2 发行版“${distro}”。`,
  wslReclaimPreserve: "保留管理区块外的 TOML 字段，替换受管模型、供应商和命令式凭据引用。",
  wslReclaimMinimal: "现有 config.toml 无法解析，将从最小受管配置重建；无法保留其中字段。",
  wslReclaimProtection: "写入前创建完整配置备份；auth.json 逐字节保持不变。",
  wslReclaimConfirmation: "再次确认：GPTEasy 将创建完整配置备份并重新接管该发行版。是否继续？",
  wslBack: "返回",
  wslClose: "关闭",
  wslRunning: "运行中",
  wslStopped: "已停止",
  ...
  providerName: "供应商名称",
  baseUrl: "服务地址",
  baseUrlPlaceholder: "请从供应商处获取 BASE_URL",
  apiKey: "API Key",
  ...
  deleteFailureFallback: "供应商未删除，请重试。",
  ...
  validationAttempt: (attempt: number, maxAttempts: number) => `第 ${attempt}/${maxAttempts} 次验证`,
  validationRetrying: "连接已断开，正在重新验证",
...
export const desktopMessages = {
  checking: "正在检查 Codex 桌面版",
  running: "Codex 桌面版运行中",
  stopped: "Codex 桌面版未运行",
  unavailable: "无法确认 Codex 桌面版状态",
  notInstalled: "未安装 Codex 桌面版",
  start: "启动 Codex",
  restart: "重启 Codex",
  ...
  cancel: "取消",
  confirmRestart: "确认重启",
  startSucceeded: "Codex 已启动。",
  restartSucceeded: "Codex 已重新启动。",
  failures: {
...
export const sessionMessages = {
  ...
  visibility: {
    ...
    executeFailed: "无法确认修复结果。暂不要重启 Codex，请查看问题日志。",
    executionFailure: {
      cliRunning: "检测到 Codex CLI，未修改会话；关闭后重新扫描。",
      unknownConsumer: "无法确认 Codex 消费者状态，未修改会话；请关闭 Codex 后重新扫描。",
      desktopRunning: "检测到 ChatGPT/Codex 桌面版，未修改会话；关闭后重新扫描。",
      targetChanged: "当前供应商状态已变化，未修改会话；请重新扫描。",
      candidateChanged: "会话记录已变化，未修改该会话；请重新扫描。",
      appServerUnavailable: "App Server 当前不可用，未修改会话；请重新扫描。",
    },
    confirm: "确认开始修复",
    executing: "正在修复",
    resultLabel: "会话可见性修复结果",
    complete: (succeeded: number) => `已修复 ${succeeded} 个会话，验证完成。`,
    partial: (succeeded: number, retryable: number) => `已修复 ${succeeded} 个，${retryable} 个待重试。`,
    failed: (retryable: number) => `本次未完成修复，${retryable} 个待重试。`,
    rescanRequired: (succeeded: number) => `已修复 ${succeeded} 个；候选已变化，请重新扫描。`,
    postWriteVerificationFailed: (succeeded: number, retryable: number) =>
      `会话写入后验证失败；已验证 ${succeeded} 个，${retryable} 个待重试。恢复清单已保留。`,
    ...
    readiness: {
      ready: "当前检查未发现执行阻断；本预览不会修改任何会话。",
      cli_running: "可检查，关闭 Codex CLI 后才能执行修复。",
      unknown_consumer: "可检查，但无法确认 Codex 消费者状态；关闭 Codex 后重新扫描。",
      desktop_running: "可检查，关闭 ChatGPT/Codex 桌面版后才能执行修复。",
      app_server_unavailable: "可检查，但 App Server 当前不可用，不能验证修复结果。",
      configuration_blocked: "可检查，但当前供应商配置状态不允许执行修复。",
      index_schema_unsupported: "可检查，但 Codex 索引 schema 未受支持，不能执行修复。",
    },
    reasonsLabel: "扫描分类与阻断原因",
    reason: {
      ...
      excluded_remote: "已排除远程 host 会话",
      excluded_derived: "已排除派生会话",
      identity_ambiguous: "身份信息不完整或存在歧义",
      no_user_event: "没有明确的用户交互事件",
      rollout_unreadable: "rollout 无法读取",
      rollout_damaged: "rollout 结构损坏",
      encrypted_content: "包含加密内容兼容性风险",
      external_configuration: "当前为外部配置",
      managed_conflict: "当前存在管理冲突",
      pending_config_operation: "存在未完成配置操作",
      target_mode_unresolved: "无法解析当前有效模式",
      environment_revision_changed: "检查期间目标 revision 已变化",
      cli_running: "Codex CLI 正在运行",
      consumer_unknown: "Codex 消费者身份无法确认",
      desktop_running: "ChatGPT/Codex 桌面版正在运行",
      app_server_unavailable: "App Server 不可用，无法验证",
      app_server_incompatible: "App Server 不兼容，无法验证",
      unsupported_index_schema: "Codex 索引 schema 未受支持",
    },
  },
