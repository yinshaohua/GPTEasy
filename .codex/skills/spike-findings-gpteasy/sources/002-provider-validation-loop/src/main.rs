...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.next().as_deref() {
        Some("validate") => {
            ...
            let result = validate(&base_url, &key, &model, &log_file);
        ...
        _ => {
            return Err(
                "usage: provider-validation-loop <validate BASE_URL MODEL LOG_FILE|mock SCENARIO PORT_FILE LOG_FILE>"
                    .into(),
            );
        }
    }
...
fn validate(base_url: &str, key: &str, model: &str, log_file: &Path) -> ValidationResult {
    ...
    let overall = (|| -> Result<(), ValidationFailure> {
        ...
        validate_tool_loop(&client, &normalized, key, model, log_file, &mut stages)?;
        Ok(())
    })();
...
fn validate_model_discovery(
    ...
    if status == StatusCode::UNAUTHORIZED || status == StatusCode::FORBIDDEN {
        record_stage(
            ...
            json!({"status": status.as_u16(), "body_excerpt": redact_excerpt(&body)}),
        );
...
fn validate_tool_loop(
    ...
    let first = post_sse(client, base_url, key, &first_payload)?;
    ...
    let arguments: Value = serde_json::from_str(&call.arguments).map_err(|error| {
        ValidationFailure::new("tool_call", format!("invalid function arguments JSON: {error}"))
    })?;
    ...
    let second = post_sse(client, base_url, key, &second_payload)?;
...
fn post_sse(
    ...
    loop {
        ...
        let bytes = reader
            .read_line(&mut line)
            .map_err(|error| ValidationFailure::new("streaming", error.to_string()))?;
        if bytes == 0 {
            break;
        }
...
fn serve_mock(
    ...
    for connection in listener.incoming() {
        ...
        let authorized = inspected["authorization_matches"].as_bool().unwrap_or(false);
...
fn inspect_mock_request(data: &[u8]) -> Value {
    ...
    let tool_output_nonce = output
        ...
        .and_then(|value| value.get("nonce").and_then(Value::as_str).map(str::to_string));
