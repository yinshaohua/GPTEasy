...
- 桌面 Codex 只删除管理区块结束标记时，兼容读取必须由完整根级受管声明和 SQLite 最后应用指纹共同证明；下一次明确应用再恢复完整边界。
...
| 标记缺失、重复或倒置 | 停止；仅“结束标记被删除且最后应用证据吻合”的窄模式允许兼容读取 |
...
桌面 Codex 的兼容例外不能放宽上述一般规则。只有开始标记位于文件开头、供应商 ID 与六个受管声明连续且结构完整，并且补回结束标记后的区块指纹与 SQLite 最后应用证据精确一致时，才把缺失结束标记视为可恢复边界。没有最后应用证据、字段或顺序漂移、额外受管字段、嵌套或重复标记仍停止。兼容读取本身不写文件；用户下一次明确应用供应商时，区块替换才恢复结束标记并保留后续桌面设置。
