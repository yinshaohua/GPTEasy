...
- 桌面 Codex 删除管理区块结束标记，或把自身设置插入受管声明与唯一的文件末尾结束标记之间时，兼容读取必须由完整根级受管声明和 SQLite 最后应用指纹共同证明；下一次明确应用再恢复紧凑边界并保留桌面设置。
...
| 标记缺失、重复或倒置 | 停止；仅“结束标记被删除或移动到文件末尾，且最后应用证据吻合”的窄模式允许兼容读取 |
...
桌面 Codex 的兼容例外不能放宽上述一般规则。只有开始标记位于文件开头、供应商 ID 与六个受管声明连续且结构完整，并且重建紧凑区块后的指纹与 SQLite 最后应用证据精确一致时，才接受结束标记缺失，或唯一结束标记被移动到文件末尾的形态。移动形态中，结束标记后只允许空白，受管前缀与结束标记之间不能出现第二个供应商 ID；完整 TOML 的根级受管字段仍必须与重建区块完全一致。没有最后应用证据、字段或顺序漂移、额外受管字段、嵌套、重复标记或结束标记后非空内容仍停止。兼容读取本身不写文件；用户下一次明确应用供应商时，区块替换才恢复紧凑边界并保留后续桌面设置。
