...
- 管理区块、备份和并发停止必须与桌面应用保持同一协议；独立 Linux 的强制设置例外按 ADR-0048 处理。
...
1. 精确扫描管理区块开始/结束整行。
2. 用户明确选择供应商后，先保存完整恢复点，再生成仅含新管理区块的最小配置；不解析或拼接旧 TOML，以免重复键、损坏边界或未知 schema 阻止新配置生效。
3. 每次切换都整体替换 `config.toml`，不因旧配置内容拒绝。
4. 同目录 `mktemp` 创建候选文件。
...
- **不要在切换时把旧 TOML 拼进新配置。** 旧文件只保存在恢复点中。
