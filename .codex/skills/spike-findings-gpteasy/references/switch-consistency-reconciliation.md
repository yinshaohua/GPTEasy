...
- 桌面 Codex 重写配置并删除结束注释，或把自身设置插入受管声明与唯一的文件末尾结束注释之间时，必须用根作用域、完整受管声明和最后应用指纹共同证明兼容；不能仅凭字段看似存在就放行。
...
桌面 Codex 可能在保存自身设置时保留文件开头的 GPTEasy 注释和全部受管 dotted keys，却删除紧随受管声明之后的结束注释，或把新增根设置放在受管声明与移动到文件末尾的结束注释之间。兼容解析只能接受已验证的窄模式：开始标记位于文件开头，供应商 ID 与六个受管声明连续且形状完全匹配，重建紧凑区块后的指纹等于 SQLite 中的最后应用指纹，且凭据指纹仍匹配。移动形态还要求结束注释唯一、位于文件末尾且其后只有空白，中间没有第二个供应商 ID，完整 TOML 的根级受管字段与重建区块完全一致。这个判断只让已证明的环境继续读取；下一次用户明确应用时重新写回紧凑边界并保留桌面设置。缺少数据库证据、字段漂移、额外受管字段、嵌套、重复标记或其它残缺仍进入 `needs_attention`。
