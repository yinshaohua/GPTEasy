...
- 桌面 Codex 重写配置并只删除管理区块结束注释时，必须用根作用域、完整受管声明和最后应用指纹共同证明兼容；不能仅凭字段看似存在就放行。
...
桌面 Codex 可能在保存自身设置时保留文件开头的 GPTEasy 注释和全部受管 dotted keys，却删除紧随受管声明之后的结束注释。兼容解析只能接受已验证的窄模式：开始标记位于文件开头，供应商 ID 与六个受管声明连续且形状完全匹配，重建结束注释后的区块指纹等于 SQLite 中的最后应用指纹，且凭据指纹仍匹配。这个判断只让已证明的环境继续读取；下一次用户明确应用时重新写回完整边界。缺少数据库证据、字段漂移、额外字段、嵌套、重复标记或其它残缺仍进入 `needs_attention`。
