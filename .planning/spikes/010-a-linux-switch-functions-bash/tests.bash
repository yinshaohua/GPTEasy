...
run_dir="${TMPDIR:-/tmp}/gpteasy-spike-010a-${UID:-0}-$$/test workspace"
...
# shellcheck source=gpteasy-switch.bash
source "$script_dir/gpteasy-switch.bash"
...
if grep -Eq '(^|[;&|[:space:]])(python3?|node|jq|perl|ruby)([;&|[:space:]]|$)' "$script_dir/gpteasy-switch.bash"; then
    no_runtime=false
else
    no_runtime=true
fi
