...
GitCode 仓库不是源码镜像，不运行构建，不单独维护版本或发布说明。正式同步与 `latest.json` 推进属于后续发布切片；本基线只建立可信输入、候选签名门禁和真实 API 冒烟路径。
...
```powershell
npm run release:manifest -- -ManifestPath <latest.json>
```
