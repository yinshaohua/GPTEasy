...
1. 拒绝草稿、预发布和非稳定 SemVer Release；匿名读取当前正式清单以阻止版本降级，首次发布则验证分发仓库 README 的 Raw 可读性。匿名读取优先使用固定分支 Raw 地址，GitCode 返回的不可变 blob 地址作为备用；遇到 418 等临时风控响应时快速重试并自动回退，避免单一下载路径阻塞同步；
