...
`source_tag` 必须是已经发布的稳定 GitHub Release；留空时使用 latest。默认使用 `.bin` 附件名上传真实 PE，验证正式流程仍使用的 Release API、匿名下载与 Raw 清单能力。`asset_extension=exe` 只用于诊断 Gitee API 的已知 `.exe` 失败边界，不是正式发布门禁。

1. 使用 form-data 与 Token 创建非正式 prerelease Release；
2. 从指定 GitHub 稳定 Release（留空时取 latest）下载真实 Windows PE 安装包，默认用 `.bin` 测试名和数值 Release ID 以 multipart/form-data 上传；
...
2. 下载 GitHub Release 中的 Windows x64 NSIS 安装包及其 `.sig`，计算大小和 SHA-256，并按标准 `.exe` 文件名生成 `SHA256SUMS.txt`；
3. 创建或复用同 Tag Gitee Release，正文采用 GitHub 中文发布说明；API 自动上传 `.sig` 和 `SHA256SUMS.txt`，但不尝试上传 `.exe`；
4. 若标准 `.exe` 尚不存在，同步器输出 `manualActionRequired`、精确文件名和 Gitee Release URL 后失败，不推进正式清单。维护者运行 `bash scripts/manual-upload-gitee-installer.sh <version>`，向导从 GitHub Release 下载权威安装包、验证 PE，打开 Gitee 编辑页指导网页上传，再匿名下载比对并触发同 Tag 同步；
5. 重跑时所有同名附件都必须经匿名下载证明大小和 SHA-256 相同；缺失的非 PE 附件才通过 API 上传。遗留 `.exe.bin`、同名内容冲突、认证或权限错误立即停止；上传响应丢失时先重新枚举附件，确认未落盘才有限重试；
6. 三个附件完成匿名校验后，最后才写入正式清单。
...
Gitee 的安装包附件名为 `GPTEasy_<version>_x64-setup.exe`，用户可直接运行。应用更新下载后仍按原 updater 签名验证完整 PE 字节，并在用户明确确认后安装；人工网页上传不绕过 `.sig`、SHA-256、匿名下载或清单最后写入门禁。

### 两阶段正式发布

GitHub Release 发布事件触发的第一次 `gitee-sync.yml` 负责准备 Gitee Release 和辅助附件。因为安装包必须由网页上传，这次工作流预期以 `manualActionRequired` 失败；失败状态是发布阻断信号，不是可忽略告警。维护者随后在 Git Bash 或 WSL2 中运行：

```bash
bash scripts/manual-upload-gitee-installer.sh 1.4.1
```

向导不读取 Gitee Token，也不重建安装包。它只下载 GitHub 已发布的权威 `.exe`，显示本地路径和 SHA-256，打开同 Tag Gitee Release 编辑页，等待维护者完成网页上传，然后通过无凭据完整 GET 比较字节。验证一致后，向导使用 `gh workflow run gitee-sync.yml -f tag=<tag>` 触发第二次自动同步并等待结果。

#### 助手与维护者的上传边界

Gitee 正式 Release 的标准 `.exe` 始终由维护者亲自在网页中上传。执行发布任务的助手不得尝试代替维护者选择文件、拖放文件、操作文件选择器或提交 Gitee Release 表单，也不得改用 API、脚本或其它自动化通道上传 `.exe`。第一次同步返回 `manualActionRequired` 后，助手只执行以下动作：

1. 核对 GitHub 权威安装包的精确本地路径、大小和 SHA-256；
2. 打开并保留输出中 `releaseUrl` 对应的 Gitee Release 编辑页；
3. 把精确 `.exe` 路径告知维护者，请维护者完成选择文件、等待上传和点击“更新”；
4. 等维护者明确回复已上传后，再匿名完整下载并核对大小、SHA-256 和 PE 内容，随后触发第二次同步；
5. 第二次同步成功且公开 `latest.md`、`.sig`、`SHA256SUMS.txt` 和 updater 签名均验证通过后，才宣布 Gitee 发布完成。

若浏览器页没有实际打开或没有保留，助手必须重新打开并确认页面存在，不能声称页面已经就绪。出现登录、验证码或平台确认时直接交由维护者处理，不尝试绕过或代操作。

对于从 `.exe.bin` 迁移的 1.4.1，维护者还必须在同一编辑页删除旧 `.exe.bin` 和引用旧文件名的 `SHA256SUMS.txt`，保留 `.sig`，再上传 `.exe`。后续新版本的第一次自动同步生成的校验文件已经引用 `.exe`，不需要删除。
...
- 正式同步以 `manualActionRequired` 失败时旧 `latest.md` 保持不变；按向导网页上传安装包后重跑同一 Tag。其它失败同样不得覆盖附件冲突或跳过校验。同一 Tag 受工作流并发锁保护，重跑成功前不得把本次国内分发记录为完成。
