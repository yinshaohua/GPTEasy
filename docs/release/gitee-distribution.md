...
4. 已存在的同名附件必须经匿名下载证明大小和 SHA-256 相同，缺失附件才上传；上传使用官方 `https://gitee.com/api/v5`、HTTP/1.1、无 `Expect: 100-continue` 的 multipart 请求；响应丢失时先重新枚举附件，确认未落盘才有限重试，认证、权限和其它确定性错误立即停止；同名内容冲突也立即停止；
