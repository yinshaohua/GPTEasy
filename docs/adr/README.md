...
- ADR-0033-0034、0041：桌面版只允许可信启动和用户确认的正常重启，Codex CLI 不受控制；GPTEasy 自有会话服务进程无窗口启动并严格回收，会话只通过官方 App Server 管理。
...
- ADR-0039：单一 GitCode 更新源内对瞬时读取失败执行有界重试，耗尽后按恢复节奏再次检查，但不回退更新源或放宽清单与签名验证。
